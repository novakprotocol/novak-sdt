...
START_EPOCH="$(date +%s)"
PROOF_START_UTC="$(date -u '+%F %T UTC')"
REPO_ROOT="$(pwd)"
...
journalctl -u "${UNIT_NAME}.service" --since "${PROOF_START_UTC}" --no-pager -n 200 \
...
if grep -Eq '(/root/\.profile|command not found|Permission denied)' "${WORKDIR}/capture/journal.txt"; then
  echo "ERROR: live timer proof journal contains shell noise" >&2
